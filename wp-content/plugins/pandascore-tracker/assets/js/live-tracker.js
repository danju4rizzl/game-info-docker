...
;(function () {
  ...
  function updateDomWithMatchData(matchId, matchData) {
    ...
    const matchElement = document.querySelector(
      `.pandascore-match-card[data-match-id='${matchId}']`
    )
    ...
    results.forEach((result) => {
      ...
      if (scoreElement) {
        ...
        if (scoreElement.textContent !== newScore) {
          ...
          const originalBackground = scoreElement.style.backgroundColor
          scoreElement.style.backgroundColor = '#10b981' // Green
          setTimeout(() => {
            scoreElement.style.backgroundColor = originalBackground
    ...
    opponents.forEach((opponent, index) => {
      ...
      if (teamLogoElements[index] && teamLogo) {
        if (teamLogoElements[index].tagName === 'IMG') {
          ...
          img.className = 'pandascore-team-logo'
          img.src = teamLogo
          img.alt = teamName
  ...
  function updateWinLoseStates(matchElement, results) {
    ...
    results.forEach((result, index) => {
      ...
      const teamRow = scoreElement?.closest('.pandascore-team-row')

      if (scoreElement && teamRow) {
